---
name: context-packet
description: Design and build context-packet DAG pipelines — graph design, shell orchestration scripts, and programmatic TypeScript integration. Use when creating AI agent workflows that pass context between nodes.
---
...
**Two interfaces:**
- **CLI** — `context-packet init|resolve|submit|read|status|hash` — any process that can shell out can participate
- **TypeScript API** — `init()`, `resolve()`, `submit()`, `read()`, `status()`
...
**The pattern for every node in a shell script:**
```bash
UPSTREAM=$(context-packet resolve <node> 2>/dev/null | jq -r '.prompt')
RESULT=$(claude -p "You are a <role>. <instructions> $UPSTREAM" 2>/dev/null)
context-packet submit <node> --status PASS --summary "<what happened>" --body "$RESULT"
```
...
If unclear, ask: "Are you designing a new pipeline, writing the orchestration script, integrating via TypeScript, or debugging an existing one?"
</routing>

<reference_index>
- references/graph-design.md — DAG patterns, edge types, fan-out/fan-in, when to use consumes vs depends_on
...
<templates_index>
- templates/graph.json — starter graph template
